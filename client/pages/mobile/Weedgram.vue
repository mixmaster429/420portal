<template>
  ...
  <div class="slide_show">
    <div
      class="weedgram-header"
      id="weedgram-header"
      ref="weedgram_header"
      v-if="model == 'user' || model == 'portal' || model == 'strain'"
    >
      <h4 class="my-1"><fa icon="arrow-left" fixed-width @click="goBack()" /> {{ username }}</h4>
    </div>
    <div
      id="media_scroll_wrapper"
      style="max-height: 100vh; overflow-y: auto"
      :style="{ 'padding-top': model == 'user' || model == 'portal' || model == 'strain' ? '40px' : 'unset' }"
      ref="scroll_wrapper"
      @scroll="handleScroll()"
    >
      <div
        :class="{ header_show: model == 'user' || model == 'portal' || model == 'strain' }"
        ref="slide_container"
        id="slide_container"
      >
          ...
          <div class="media_header">
            <div class="user_logo">
              <a :href="'/' + item.user.username">
                <img
                  :src="serverUrl(item.user.profile_pic ? item.user.profile_pic.url : default_logo)"
                  alt="Profile picture"
                />
              </a>
            </div>
              ...
              <div class="user_holder d-flex align-items-center">
                <div class="user_data">
                  ...
                  <span
                    ...
                    v-show="logged_user_id != item.user_id"
                    v-if="!item.isfollower"
                    >Follow</span
                  >
                  <img
                  ...
                  <div class="user_icons" v-if="item.user.type != 'user'">
                    <img
                      ...
                      v-if="item.user.store_type === 2 || item.user.store_type === 3"
                      alt
                    />
                    <img
                      src="~assets/imgs/dispensary.png"
                      v-if="item.user.store_type === 1 || item.user.store_type === 3"
                      alt
                    />
          ...
          <div class="media_body">
            <div class="slide_image" @click="user ? likeMedia(item) : () => false">
              ...
              <video
                ...
                alt
                v-if="item.type == 'video' && $route.params.start_index === index + 1"
            ...
            <div
              class="menu-panel"
              v-if="item.menu && item.menu.is_active"
              :class="{ 'brand-menu': item.user.type == 'brand' }"
            >
              <p class="menu-category">
                <span>{{
                  item.menu.category.price_type == 2 ? 'Flower | ' + item.menu.category.name : item.menu.category.name
                }}</span>
              ...
              <p class="price_data mb-0" v-if="item.menu.portal && item.menu.portal.type == 'company'">
                <span class="text-white pr-2" v-if="item.menu.price_each != null"
                  >{{ item.menu.price_each }}<sup class="text-420">each</sup></span
                >
                <span class="text-white pr-2" v-if="item.menu.price_half_gram != null"
                  >{{ item.menu.price_half_gram }}<sup class="text-420">1/2g</sup></span
                >
                <span class="text-white pr-2" v-if="item.menu.price_gram != null"
                  >{{ item.menu.price_gram }}<sup class="text-420">g</sup></span
                >
                <span class="text-white pr-2" v-if="item.menu.price_eighth != null"
                  >{{ item.menu.price_eighth }}<sup class="text-420">1/8</sup></span
                >
                <span class="text-white pr-2" v-if="item.menu.price_quarter != null"
                  >{{ item.menu.price_quarter }}<sup class="text-420">1/4</sup></span
                >
                <span class="text-white pr-2" v-if="item.menu.price_half_oz != null"
                  >{{ item.menu.price_half_oz }}<sup class="text-420">1/2</sup></span
                >
                <span class="text-white pr-2" v-if="item.menu.price_oz != null"
                  >{{ item.menu.price_oz }}<sup class="text-420">oz</sup></span
                >
              </p>
            </div>
          </div>
          ...
          <div class="media_description">
            <p v-if="item.description">
              ...
              <span>
                ...
                <span
                  class="text-420 ml-1 btn-readmore"
                  @click="showDescription(index)"
                  v-show="item.description_expanded != true && item.description.length > 140"
                  >Read More</span
                >
              </span>
            </p>
            <router-link
              :to="{ name: 'comment', params: { target: item.id, media: item } }"
              v-if="item.count_comment > 0"
            >
              <p class="allcomments mb-1">
                View all <span class="text-primary">{{ item.count_comment }}</span> comments
              </p>
            </router-link>
            <div class="v_comment_box">
              <div class="proifle-img">
                <img
                  class="userlogo"
                  :src="serverUrl(user.profile_pic.url)"
                  v-if="user !== null && user.profile_pic !== null"
                  alt
                />
        ...
        this.axios.post(uri, params).then((response) => {
          ...
          if (widnow.user != this.posts[index].user_id) {
            let noti_fb = firebase
              .database()
              .ref('notifications/' + this.posts[index].user_id)
              .push()
            noti_fb.set({
          ...
          this.axios.post(uri, params).then((response) => {
            ...
            if (item.user_liked) {
              ...
              if (this.user.id != item.user_id) {
                let noti_fb = firebase
                  .database()
                  .ref('notifications/' + item.user_id)
                  .push()
                noti_fb.set({
      ...
      if (this.user) {
        ...
        this.axios.post(uri, params).then((response) => {
          ...
          for (let i = 0; i < this.posts.length; i++) {
            if (this.posts[i]['user']['id'] === id) this.posts[i]['isfollower'] = !this.posts[i]['isfollower']
          }
        })
      } else {
        $('#loginmodal').modal('show')
