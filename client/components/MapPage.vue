<template>
  <div class="map-container row">
    <div class="search_bar col-md-12">
      <div class="mobile">
        <h4 class="text-420 results_count" @click="showList()" v-if="!loading && !show_map">
          {{ results_count }} Results
        </h4>
        <h4 class="text-420 results_count" @click="showMap()" v-if="!loading && show_map && !show_list">
          {{ results_count }} Results
        </h4>
    ...
    <div class="col-12 filter_mobile pt-2" id="filter_mobile" v-if="$device.isMobile">
      <div class="row">
        <div class="col-6 pt-3">
          <div class="custom-control custom-checkbox filter-input" style="padding-top: 3px">
            <input
              ...
              id="mobile_filter_storefront"
              value="1"
              v-model="top_filter.business_type"
            />
          ...
          <div class="custom-control custom-checkbox filter-input" style="padding-top: 3px">
            <input
              type="checkbox"
              class="custom-control-input"
              id="mobile_filter_recreational"
              value="recreational"
              v-model="top_filter.filters"
            />
        ...
        <div class="col-6">
          ...
          <div
            class="form-group floating-label floating-multiselect mobile_filter_portal"
            :class="{ selected: top_filter.portal }"
          >
            <multiselect
              class="filter_portal"
      ...
      <div class="row">
        <div class="col-6">
          <div
            class="custom-control custom-radio filter-input"
          ...
          >
            <input
              ...
              class="custom-control-input"
              name="category"
              :id="'mobile_menu_flowers_' + category.slug"
              :value="category.id"
              v-model="top_filter.category"
              @change="selectCategory"
            />
            <label class="custom-control-label" :for="'mobile_menu_flowers_' + category.slug">{{
          ...
          <div class="pt-5">
            <div class="form-group floating-label">
              <input
                ...
                id="mobile_search_products"
                placeholder=" "
                :value="top_filter.menu_strain"
                @input="(e) => (top_filter.menu_strain = e.target.value)"
                @focus="searchPortalOpen()"
                @blur="searchPortalClose()"
              />
            ...
            <div class="form-group floating-label" v-if="top_filter.selected_category">
              <select
                class="form-control floating-select mt-2"
                id="mobile_weight_amount"
                v-model="top_filter.menu_price_type"
              >
    ...
    <div class="col-md-12" id="portal_filter" v-else>
      <div class="row">
        <div class="col-12 d-flex justify-content-center align-items-center">
          <img
            src="/imgs/h_icon1.png"
            width="35"
            height="35"
            style="margin-top: -8px; margin-right: 7px"
            alt="Marijuana Dispensaries and Deliveries"
          />
      ...
      <div class="row">
        ...
        <div class="col-md-4">
          <div class="filter-dropdown">
            <img
              src="/imgs/search_option.png"
              class="d-block mx-auto"
              width="30"
              @mouseover="open_menus = !open_menus"
            />
                    ...
                    >
                      <div class="custom-control custom-radio filter-input">
                        <input
                          ...
                          class="custom-control-input"
                          name="category"
                          :id="'menu_flowers_' + category.slug"
                          :value="category.id"
                          v-model="top_filter.category"
                          @change="selectCategory"
                        />
                        <label class="custom-control-label" :for="'menu_flowers_' + category.slug">{{
                          category.name | category_name
                        }}</label>
                      </div>
                    </a>
                    ...
                    <div class="pt-md-5" style="width: 170px">
                      <div class="form-group floating-label">
                        <input
                          ...
                          placeholder=" "
                          id="filter_product"
                          v-model="top_filter.menu_strain"
                          autocomplete="off"
                        />
                      ...
                      <div class="form-group floating-label" v-if="top_filter.selected_category">
                        ...
                        >
                          <option value="price_each">Each</option>
                          <option value="price_half_gram" v-show="top_filter.selected_category.price_type == 1">
                            0.5g
                          </option>
                          <option value="price_gram" v-show="top_filter.selected_category.price_type > 0">g</option>
                          <option value="price_eighth" v-show="top_filter.selected_category.price_type == 2">
                            1/8
                          </option>
                          <option value="price_quarter" v-show="top_filter.selected_category.price_type == 2">
                            1/4
                          </option>
                          <option value="price_half_oz" v-show="top_filter.selected_category.price_type == 2">
                            1/2
                          </option>
                          <option value="price_oz" v-show="top_filter.selected_category.price_type == 2">oz</option>
                        </select>
                      ...
                      <h5 class="mt-3">
                        <a href="javascript:;" class="text-420" @click="resetCategory()">Reset Options</a>
                      </h5>
                    </div>
                  </div>
                </div>
    ...
    <div class="col-md-3 map_list" :style="{ height: is_ios ? 'calc(100vh - 272px)' : 'calc(100vh - 214px)' }">
      <div class="row">
        ...
        <div
          class="w-100 portal-container"
          v-for="(item, index) of portals"
          :key="index"
          v-if="filter == null || filter == index"
        >
          <div
            class="pt_list"
            v-if="item.menus.length == 0"
            @click="goPortal(item.username)"
            @mouseover="hover_portal = item.id"
            @mouseleave="hover_portal = null"
          >
            <div class="pt_data">
              <div class="pt_logo">
          ...
          <div
            ...
            v-for="(m_item, m_index) of item.menus"
            :key="m_index"
            v-if="item.menus.length != 0"
            @click="goPortal(item.username)"
            @mouseover="hover_portal = item.id"
          ...
          >
            <div class="result-media">
              <div class="media" v-if="m_item.media">
                ...
                <video
                  v-if="m_item.media.type == 'video'"
                  :src="serverUrl(m_item.media.url)"
                ...
                >
                  ...
                  <source v-bind:src="serverUrl(m_item.media.url)" type="video/ogg" />
                  Your browser does not support the video tag.
                </video>
                <img
                  class="video__tag__mobile"
                  v-if="m_item.media.type === 'video'"
                  src="https://i.imgur.com/88aBgwi.png"
                  alt=""
                />
              </div>
            ...
            <div class="result-data">
              <p class="category-strain">
                <span class="category">{{
                  m_item.category.price_type == 2 ? 'Flower | ' + m_item.category.name : m_item.category.name
                }}</span>
              ...
              <div class="price_data mb-0 mt-1">
                <span class="text-white pr-2" v-if="m_item.price_each != null"
                  >{{ m_item.price_each }}<sup class="text-420">each</sup></span
                >
                <span class="text-white pr-2" v-if="m_item.price_half_gram != null"
                  >{{ m_item.price_half_gram }}<sup class="text-420">1/2g</sup></span
                >
                <span class="text-white pr-2" v-if="m_item.price_gram != null"
                  >{{ m_item.price_gram }}<sup class="text-420">g</sup></span
                >
                <span class="text-white pr-2" v-if="m_item.price_eighth != null"
                  >{{ m_item.price_eighth }}<sup class="text-420">1/8</sup></span
                >
                <span class="text-white pr-2" v-if="m_item.price_quarter != null"
                  >{{ m_item.price_quarter }}<sup class="text-420">1/4</sup></span
                >
                <span class="text-white pr-2" v-if="m_item.price_half_oz != null"
                  >{{ m_item.price_half_oz }}<sup class="text-420">1/2</sup></span
                >
                <span class="text-white pr-2" v-if="m_item.price_oz != null"
                  >{{ m_item.price_oz }}<sup class="text-420">oz</sup></span
                >
              </div>
            </div>
          </div>
      ...
      >
        <div class="modal-content">
          <div class="modal-body p-0" v-if="details">
            <div class="pt_list" v-if="details && details.menus.length == 0" @click="goPortal(details.username)">
              <div class="pt_data">
                ...
                <div class="pt_details" style="width: calc(100% - 60px)">
                  ...
                  <p class="pt_address">
                    {{ details ? details.city : '' }}, {{ details ? details.state : '' }},
                    {{ details ? details.postal : '' }}
                  </p>
                  <p class="distance">
                    <span class="shop_status" style="color: #93e612" v-if="details && details.shop_status == 2"
                      >Open</span
                    >
                    <span class="shop_status" style="color: red" v-else>Closed</span>
              ...
              <div v-if="details.menus.length">
                <div class="pt_menu w-100" v-for="(m_item, m_index) of details.menus" :key="m_index">
                  ...
                  <p class="price_data mb-0">
                    <span class="text-white px-2" v-if="m_item.price_each != null"
                      >{{ m_item.price_each }}<sup class="text-420">each</sup></span
                    >
                    <span class="text-white px-2" v-if="m_item.price_half_gram != null"
                      >{{ m_item.price_half_gram }}<sup class="text-420">1/2g</sup></span
                    >
                    <span class="text-white px-2" v-if="m_item.price_gram != null"
                      >{{ m_item.price_gram }}<sup class="text-420">g</sup></span
                    >
                    <span class="text-white px-2" v-if="m_item.price_eighth != null"
                      >{{ m_item.price_eighth }}<sup class="text-420">1/8</sup></span
                    >
                    <span class="text-white px-2" v-if="m_item.price_quarter != null"
                      >{{ m_item.price_quarter }}<sup class="text-420">1/4</sup></span
                    >
                    <span class="text-white px-2" v-if="m_item.price_half_oz != null"
                      >{{ m_item.price_half_oz }}<sup class="text-420">1/2</sup></span
                    >
                    <span class="text-white px-2" v-if="m_item.price_oz != null"
                      >{{ m_item.price_oz }}<sup class="text-420">oz</sup></span
                    >
                  </p>
                </div>
            ...
            <div
              class="result-container"
              v-for="(m_item, m_index) of details.menus"
              :key="m_index"
              v-if="details.menus.length != 0"
              @click="goPortal(details.username)"
            >
              <div class="result-media">
                <div class="media" v-if="m_item.media">
                  <img v-if="m_item.media.type == 'image'" :src="serverUrl(m_item.media.url)" alt="" />
              ...
              <div class="result-data">
                ...
                <h5 class="company-name">
                  <img
                    ...
                    src="/imgs/dispensary.png"
                    alt
                    v-if="details.store_type == 1 || details.store_type == 3"
                  />
                  <img
                    class="store-type-img"
                    src="/imgs/delivery.png"
                    alt
                    v-if="details.store_type == 2 || details.store_type == 3"
                  />
      ...
      <div class="modal-dialog modal-lg border-0">
        <div class="modal-content">
          <div class="modal-header bg-white">
            <button type="button" class="close text-420" data-dismiss="modal">
          ...
          <div class="modal-body" style="background: white !important">
            <div class="row">
              <div class="col-md-12 text-center">
                ...
                <p class="pt-3">
                  Our map displays both
                  <span style="font-size: 18px"
                    >marijuana dispensaries and deliveries in
                    <font style="text-transform: capitalize">{{ city }}, {{ state }}</font> </span
                  >. Find your local cannabis dispensary and delivery nearest you. We display both recreational and
                  medical marijuana dispensaries and deliveries in
                  <font style="text-transform: capitalize">{{ city }}, {{ state }}</font
                  >. So if you're looking for weed products, you have come to the right place!
                </p>
              </div>
            </div>
          </div>
