<template>
  <form @submit.prevent="type()" :id="'form_media_' + mode" class="form-media mt-3">
    <div class="form-group text-center">
      ...
      <div v-if="mediaData.length > 0">
        ...
        >
          ...
          <source :src="mediaData" type="video/ogg" />
    ...
    <div
      ...
      @click="handleScroll"
      v-show="!$device.isMobile || ($device.isMobile && mediaData != '')"
    >
      <textarea
        v-model="description"
        :id="'media_description_' + mode"
        class="form-control media-description floating-textarea"
        rows="2"
        placeholder=" "
      ></textarea>
    ...
    <div
      ...
      v-show="!$device.isMobile || ($device.isMobile && mediaData != '')"
      v-if="auth_user && auth_user.type == 'user'"
    >
      <label
        v-bind:class="[floatUserLabel ? 'multi-select__label float-label' : 'multi-select__label']"
        for="users__select"
        >Tag Users</label
      >
      <multiselect
        v-model="taggedUsers"
        class="users__select floating-input floating-select"
        label="name"
        ...
        :showNoOptions="false"
        :hide-selected="true"
        @search-change="asyncFind($event, 'user')"
        @open="onOpen('user')"
        @select="floatUserLabel = true"
        @close="taggedUsers.length >= 1 ? (floatUserLabel = true) : (floatUserLabel = false)"
        @remove="taggedUsers.length >= 1 ? '' : (floatUserLabel = false)"
      >
    ...
    <div
      ...
      style="position: relative"
      v-show="!$device.isMobile || ($device.isMobile && mediaData != '')"
      v-if="!from && auth_user && auth_user.type == 'user'"
    >
      <label v-bind:class="[floatCompanyLabel ? 'multi-select__label float-label' : 'multi-select__label']"
        >Tag Companies</label
      >
      <multiselect
        v-model="taggedCompanies"
        class="companies__select floating-input floating-select"
        v-bind:class="[taggedCompanies.length >= 1 ? 'select__disable' : '']"
        label="name"
        track-by="id"
        placeholder
        open-direction="bottom"
        :options="companies"
        :searchable="isCompanySearchable"
        :multiple="true"
        ...
        :limit="1"
        :max="1"
        :max-height="companyMaxHeight"
        :show-no-results="false"
        :showNoOptions="false"
        :hide-selected="true"
        @search-change="asyncFind($event, 'company')"
        @open="onOpen('company')"
        @select="onSelect('company')"
        @close="onClose('company')"
        @remove="onRemove('company')"
      >
        <template slot="tag" slot-scope="{ option, remove }">
          <span class="custom__tag">
            <span>{{ option.name }}</span>
    ...
    >
      <label
        v-bind:class="[floatStrainLabel ? 'multi-select__label float-label' : 'multi-select__label']"
        for="strains__select"
        >Tag Marijuana Strain</label
      >
      <multiselect
    ...
    <div class="form-group" v-show="!$device.isMobile || ($device.isMobile && mediaData != '')">
      <button
        class="btn btn-primary mt-2 d-block px-3 mx-auto d-flex align-items-center"
  ...
  methods: {
    fetchData(id) {
      ...
      if (this.mediaType === 'video') {
        this.mediaData = this.serverUrl(this.mainData.url) + '#t=0.05'
      } else {
    ...
    update() {
      ...
      this.axios
        ...
        .catch((e) => {
          this.$toast.error('Error editing media', '')
          this.loading = false
        })
    },
    save() {
      ...
      this.axios
        ...
        .catch((e) => {
          this.$toast.error('Error adding media', '')
          console.log('error')
          this.loading = true
        })
    },
    ...
    previewImage(event) {
      ...
      if (input.files && input.files[0]) {
        ...
        if (this.mediaType === 'video') {
          this.mediaData = URL.createObjectURL(this.media) + '#t=0.1'
        } else {
